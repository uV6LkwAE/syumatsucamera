...
- 画像アップロードAPI経由で記事内画像を保存する。
  ...
  - Jodit 側の許可拡張子も `jpg`, `jpeg`, `gif` に限定する。
...
C. GIF は加工せず、そのままオリジナル保存先と公開用保存先へ保存する。
D. 記事本文画像と `thumbnail_request.mode=use_uploaded` のサムネイルは、同じ画像処理パイプラインへ通す。
E. `thumbnail_request.mode=use_default`, `generate_from_title`, `keep_current` はこの画像処理パイプラインへ通さない。
F. GIF 以外は Exif 取得後、必要な場合のみリサイズと透かし挿入を行う（ファイル名はアップロード時にUUID確定済み）。
G. `resize=true` でも元ファイルサイズが `500KB` 以下なら、リサイズのみスキップする。
H. `500KB` 以下でリサイズのみスキップした場合でも、透かしオプションは通常通り実行する。
I. リサイズ時は長辺 `2048px` 上限、拡大なしとする。
J. リサイズ・透かし処理後に公開用画像が `500,000 bytes` を超える場合、JPEG は品質を二分探索で下げる。
K. 品質探索範囲は `CMS_ARTICLE_IMAGE_QUALITY_LOW=50`, `CMS_ARTICLE_IMAGE_QUALITY_HIGH=90` とする。
L. 上限サイズ以下を満たす最大品質を選択し、満たす品質が見つからない場合は `CMS_ARTICLE_IMAGE_QUALITY_LOW` で保存する。
M. PNG / WebP はアップロード許可対象外とする。
N. `exif_watermark` は左下の半透明黒帯へ描画し、GPS 情報は使わない。
O. `site_logo_watermark` は右下配置、ロゴ幅は長辺の `12%`、透明度は `60%` とする。
P. 処理成功した画像は、その都度オリジナル保存先と公開用保存先へ移動してよい。
Q. オリジナルの画像は別のディレクトリに保存し、公開しない。
R. 画像処理が一部失敗しても、成功した画像は保存済みとしてよい。
S. 画像処理成功時に、ジョブ側で本文HTML内の `tmp` パスを公開用パスへ書き換える。
T. 公開中記事の更新では、処理完了まで旧公開版を出し続ける。
U. 記事編集APIから渡された削除対象画像について、原本/公開用画像を削除し、関連する `MEDIA_ASSET` を整理する。
V. 正常終了・異常終了のどちらでも、最終処理で対象 `tmp/{lock_token}` ディレクトリ配下をクリーンアップする。
W. `tmp/{lock_token}` クリーンアップ完了後に、ジョブ自身が記事ロックを解除する（成功/失敗時ともに実施）。
X. 画像処理ジョブが完了するまで記事は公開不可とする。
Y. 画像処理が1件でも失敗した場合、記事は公開しない。
Z. CMS側では対象記事へ警告バッジを表示し、記事保存フローログを参照できるようにする。
AA. どの画像が失敗したかは記事保存フローログテーブルへ残す。
2. OGPレコード作成
